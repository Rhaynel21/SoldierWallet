...
const initialState = {
  ...
  currentId: null,
  lastPhone: '',
  accounts: [], // no seeded users — everyone signs up (stored in Firestore)
}
...
function loadState() {
  try {
    ...
    return { ...initialState, ...parsed, accounts: parsed.accounts || [] }
  } catch {
    return initialState
  }
}
...
// last 10 digits of a phone (matches 09xx, +639xx, 9xx formats)
...
// Find another account by account no / mobile / serial / name
...
export function WalletProvider({ children }) {
      ...
      (snap) => {
        if (snap.empty && !seededRef.current) {
          ...
          state.accounts.forEach((a) =>
            setDoc(doc(db, ACCOUNTS_COL, a.id), a).catch(() => {}),
          )
          return
        }
        if (!snap.empty) {
          ...
          dispatch({ type: 'REPLACE_ACCOUNTS', payload: { accounts: snap.docs.map((d) => d.data()) } })
        }
      },
  ...
  // Firestore: write-through every account change (stores all data)
  useEffect(() => {
    if (!firebaseEnabled || !db) return
  ...
  const value = useMemo(() => {
    const cur = getCurrent(state) || {}
    return {
      // current account, spread so existing components keep working
      user: {
        name: cur.name || '',
        rank: cur.rank || '',
        serialNo: cur.serialNo || '',
